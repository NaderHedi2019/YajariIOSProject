...
class ItemView : FrameLayout {
    ...
    private fun generateViews(styles: TypedArray) {
        val defaultHeight = Utils.getPixels(context, DEFAULT_BAR_HEIGHT.toFloat()).toFloat()
        val defaultOtpTextSize = Utils.getPixels(context, DEFAULT_OTP_TEXT_SIZE.toFloat()).toFloat()
        ...
        val barMargin = styles.getDimension(R.styleable.OtpTextView_bar_margin, Utils.getPixels(context, 0f).toFloat())
