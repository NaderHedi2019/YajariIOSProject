...
import com.app.yajari.databinding.FragmentMyDonationBinding
...
class MyDonationFragment : BaseFragment<FragmentMyDonationBinding>(),MyDonationAdapter.MyDonationListener {
    private var isSwipeRefresh = false
    private var isLoader = false
    private lateinit var lManager: RecyclerView.LayoutManager
    private lateinit var scrollListener: EndlessScrollListener
    private var donationAdapter: MyDonationAdapter? = null
    ...
    override fun getViewBinding()= FragmentMyDonationBinding.inflate(layoutInflater)
    ...
    override fun click() {
        binding.apply {
            noDataLayout.noDataTV.text=getString(R.string.no_donation_chat_found)
...
//            }
    ...
    override fun setUpObserver() {
        ...
        setRVScrollListener()
        callGetChatList(true)
        chatViewModel.chatDonationListResponse.observe(this) { response ->
    ...
    private fun setMyDonationRV()
    {
        binding.myDonationRV.apply {
            lManager = LinearLayoutManager(context, RecyclerView.VERTICAL, false)
            layoutManager = lManager
            donationAdapter = MyDonationAdapter(requireContext(),this@MyDonationFragment)
...
//                        (lManager as LinearLayoutManager).findFirstCompletelyVisibleItemPosition() == 0
        ...
        binding.myDonationRV.addOnScrollListener(scrollListener)
    }

    private fun callGetChatList(isLoadMore: Boolean) {
        isLoader=isLoadMore
        chatViewModel.donationChatList(
            mapOf(
    ...
    private fun bindData(requestData: MutableList<ChatListResponse.Data>?) {
        if (requestData!!.isEmpty() && isLoader) {
            ...
            binding.myDonationRV.gone()
            return
        }
        binding.noDataLayout.root.gone()
        binding.myDonationRV.visible()
        chatList.addAll(requestData)
        donationAdapter?.removeLoading()
        donationAdapter?.addData(requestData, isSwipeRefresh)
...
//        }
    ...
    override fun onItemDonationClick(data: ChatListResponse.Data) {
        if(data.friendRequest!!.isEmpty())
        {
            ...
            intent.putExtra(Constant.OBJECT_TYPE, Constant.DONATION)
