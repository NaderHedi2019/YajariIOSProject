...
import com.app.yajari.databinding.FragmentRequestProfileBinding
...
class RequestProfileFragment : BaseFragment<FragmentRequestProfileBinding>(),MyAnnouncementAdapter.MyAnnouncementListener {
    ...
    private var requestList = mutableListOf<MyAnnouncementResponse.Data>()
    ...
    override fun getViewBinding()= FragmentRequestProfileBinding.inflate(layoutInflater)

    companion object{
        fun newInstance(type: String): RequestProfileFragment {
            val data= Bundle()
            data.putString("type",type)
            return RequestProfileFragment().apply{
                arguments = data
            }
        }
    }
    ...
    private fun setUpRequestRV() {
        binding.noDataLayout.noDataTV.text=getString(R.string.no_request_found)
        binding.requestRV.run {
            lManager = GridLayoutManager(context, 2)
            layoutManager = lManager
            donationAdapter = MyAnnouncementAdapter(requireContext(),this@RequestProfileFragment)
        ...
        donationAdapter?.setObjectType(Constant.REQUEST)
    }
    ...
    private fun callMyAnnouncement(isLoadMore: Boolean) {
        ...
        profileViewModel.myAnnouncement(
            mapOf(
                ...
                Constant.OBJECT_TYPE to Constant.REQUEST,
