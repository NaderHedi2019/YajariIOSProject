...
import com.app.yajari.databinding.AdapterMyRequestBinding
...
class MyRequestAdapter(private var mContext: Context, private var listener:MyRequestListener) :  RecyclerView.Adapter<RecyclerView.ViewHolder>(),AutoUpdatableAdapter {
    ...
    inner class MyViewHolder(val binding : AdapterMyRequestBinding) : RecyclerView.ViewHolder(binding.root)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when(viewType){
            1 -> MyViewHolder(AdapterMyRequestBinding.inflate(LayoutInflater.from(parent.context),parent,false))
        ...
        if (holder is MyViewHolder) {
            ...
            holder.binding.apply {
                val params: ViewGroup.MarginLayoutParams =
                    myRequestCL.layoutParams as ViewGroup.MarginLayoutParams
                ...
                myRequestCL.layoutParams = params
                ...
                when (data.status) {
                    Constant.AVAILABLE -> {
                        ...
                        requestTypeTV.setCompoundDrawablesWithIntrinsicBounds(R.drawable.ic_available_announcement,0,0,0)
                        requestTypeTV.setTextColor(mContext.getClr(R.color.color_available))
                    }
                    Constant.RESERVED -> {
                        requestTypeTV.text=mContext.getStr(R.string.reserved)
                        requestTypeTV.setCompoundDrawablesWithIntrinsicBounds(R.drawable.ic_reserved_announcement,0,0,0)
                        requestTypeTV.setTextColor(mContext.getClr(R.color.color_reserved))
                    }
                    Constant.FINALIZED -> {
                        requestTypeTV.text=mContext.getStr(R.string.finalized)
                        requestTypeTV.setCompoundDrawablesWithIntrinsicBounds(R.drawable.ic_announcement,0,0,0)
                        requestTypeTV.setTextColor(mContext.getClr(R.color.color_finalize))
                    }
                }
            }

            holder.binding.myRequestCL.setSafeOnClickListener {
                listener.onItemMyRequestClick(data)
            }
        }
    }
    ...
    interface MyRequestListener{
        fun onItemMyRequestClick(data: ChatListResponse.Data)
    }
